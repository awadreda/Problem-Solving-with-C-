...
int GetDifferenceInDays(stDate Date1, stDate Date2, bool IncludeEndDay = false)
{
  ...
  else
  {
    ...
    return -1 * (IncludeEndDay ? ++Days : Days);
  }

  return IncludeEndDay ? ++Days : Days;
}

void SwabDate(stDate &Date1, stDate &Date2)
{
  stDate temp;
  temp = Date1;
  Date1 = Date2;
...
int main()
{
  ...
  SwabDate(Date1,Date2);

  cout << "Date After Adding One Day : " << Date1.Day << "/" << Date1.Month << "/" << Date1.Year << "\n";
  cout << "Date After Adding One Day : " << Date2.Day << "/" << Date2.Month << "/" << Date2.Year << "\n";
